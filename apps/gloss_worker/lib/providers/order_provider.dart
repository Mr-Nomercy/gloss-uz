...
class OrderNotifier extends StateNotifier<OrderState> {
  ...
  Future<void> loadOrderDetail(String orderId) async {
    ...
    try {
      final response = await _dio.get('/provider/orders/$orderId');
  ...
  Future<bool> acceptOrder(String orderId) async {
    ...
    try {
      await _dio.patch('/provider/orders/$orderId/accept');
  ...
  Future<bool> rejectOrder(String orderId) async {
    state = state.copyWith(isLoadingAction: true, error: null);
    try {
      await _dio.patch('/provider/orders/$orderId/reject');
      state = state.copyWith(isLoadingAction: false);
      await loadOrders();
      return true;
    } on DioException catch (e) {
  ...
  Future<bool> updateOrderStatus(String orderId, String status) async {
    state = state.copyWith(isLoadingAction: true, error: null);
    try {
      await _dio.patch('/provider/orders/$orderId/status', data: {'status': status});
      state = state.copyWith(isLoadingAction: false);
      await loadOrders();
  ...
  Future<bool> cancelOrder(String orderId, String reason) async {
    state = state.copyWith(isLoadingAction: true, error: null);
    try {
      await _dio.patch('/provider/orders/$orderId/cancel', data: {'reason': reason});
      state = state.copyWith(isLoadingAction: false);
      await loadOrders();
      return true;
    } on DioException catch (e) {
